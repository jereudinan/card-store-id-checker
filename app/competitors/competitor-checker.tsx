...
type Result = { stores: Store[]; scannedCount: number; radius: number; keyword: string };
    ...
    <section className="competitor-tool" aria-labelledby="competitor-tool-title">
      ...
      <div className="competitor-results" aria-live="polite">
        {!result ? <div className="competitor-placeholder"><strong>주변 경쟁업체를 확인해 보세요</strong><p>선택한 반경 안에서 입력한 업종과 관련된 가게를 찾아드립니다.</p></div> : <>
          <header><div><span>{result.radius === 1000 ? "1km" : `${result.radius}m`} 반경</span><h2><strong>{result.stores.length}</strong>곳을 찾았습니다</h2></div><p>공개된 상가업소 {result.scannedCount}곳 중 ‘{result.keyword}’ 관련 업종</p></header>
